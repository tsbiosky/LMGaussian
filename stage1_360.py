...
from utils.loss_utils import l1_loss, ssim,  pearson_depth_loss, local_pearson_loss, lpips_loss, lpr_loss
from utils.graphics_utils import vis_surface_normal_inverse, vis_surface_normal, load_pointcloud, interpolate_camera_poses, interpolate_camera_poses_test
from gaussian_renderer import render, network_gui, render_point
import sys
...
from utils.graphics_utils import point_double_to_normal, depth_double_to_normal, random_mask
...
def training(dataset, save_dir, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = Scene(dataset, gaussians, opt.opt_train_depth, opt.opt_train_normal)
    ...
    if W1 >= 1600 or dataset.resolution != 1:
        trans = transforms.Resize([H, W], antialias=True)    
    print("start rendering point-rasterized images----------------------")
    ...
    for iteration in range(first_iter, opt.iterations + 1):       
        ...
        rendered_image, viewspace_point_tensor, visibility_filter, radii = (
                                                                    ...
                                                                    render_pkg["radii"])
        rendered_depth: torch.Tensor = render_pkg["expected_depth"]
        ...
        if iteration < 1000:
            torchvision.utils.save_image(random_mask(rendered_image.clone().detach()), os.path.join(save_dir, f'sample_{iteration}.png'))
        loss = 0 
        ...
        L_pr = lpips_loss(image, point_image, loss_fn, device=dataset.data_device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3000, 6000]) # 9000, 12000
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3000, 6000]) # 9000, 12000
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[3000, 6000]) # 9000, 12000
    ...
    print("\nTraining complete.")
    print(f"stage one: {end - start:.4f} 秒")
