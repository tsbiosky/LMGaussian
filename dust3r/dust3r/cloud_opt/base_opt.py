...
from dust3r.utils.geometry import inv, geotrf, pearson_depth_loss, local_pearson_loss, loss_depth_smoothness
...
class BasePCOptimizer (nn.Module):
    ...
    def get_masks_depth(self):
        ...
        for i, pts3d in enumerate(self.depth_to_pts3d()):
            msk_i = (depthmaps[i] > 0) 
            threshold_depth = torch.median(depthmaps[i]) * 1.5
    ...
    def forward(self, iter = 0, ret_details=False):
        ...
        if self.depth_path != None:
            ...
            gt_depths = read_and_resize_depths(self.depth_path, (depths[0].shape[1], depths[0].shape[0]))
            gt_depths = torch.tensor(np.array(gt_depths), dtype=torch.float32).to(self.device)
            loss_depth = Loss_depth_calculate(depths, gt_depths, iter) / len(depths)
        else:
            loss_depth = 0
    ...
    def compute_global_alignment(self, init=None, niter_PnP=10, **kw):
        if init is None:
            ...
            init_fun.init_minimum_spanning_tree_glm(self, niter_PnP=niter_PnP)
...
def read_and_resize_depths(folder_path, target_size=(512, 288)):
    ...
    for file_name in file_names:
        ...
        depth_image = cv2.imread(file_path, cv2.IMREAD_GRAYSCALE)
        resized_image = cv2.resize(depth_image, target_size)
        depth_images.append(resized_image)

    return depth_images



def global_alignment_loop(net, lr=0.01, niter=200, schedule='cosine', lr_min=1e-6):
...
def Loss_depth_calculate(depths, gt_depths, iter):
    loss_depth = 0
    for (depth, gt_depth) in zip(depths, gt_depths):
        depth = depth.unsqueeze(0)
        gt_depth = gt_depth.unsqueeze(0)
        pearson_loss = pearson_depth_loss(depth[0], gt_depth[0])
        lp_loss = local_pearson_loss(depth[0], gt_depth[0], 32, 0.5) 
        if iter > 100:
            lp_loss += loss_depth_smoothness(depth[None,...], gt_depth[None,...])
        loss_depth += (pearson_loss + lp_loss * 5 ) 
    return loss_depth 